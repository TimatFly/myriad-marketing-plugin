...
class myriad_integration_plugin
{
	...
	public function register_myriad_settings()
	{
		...
		register_setting( "myriad_settings", "MyriadCardPaymentID" );
		...
		add_settings_field('myriad_settings_cardid', 'Myriad SOAP Card Payment Type ID', array(&$this,'plugin_setting_cc_string'), 'myriad_settings', 'myriad_settings_main');
	...
	public function plugin_setting_cc_string() {
		$options = get_option('MyriadCardPaymentID');
		echo "<input id='myriad_soap_cc_input_field' name='MyriadCardPaymentID[text_string]' size='40' type='text' value='{$options['text_string']}' />";
	} 
	...
	<div class="options_group">
    					<?php
						  woocommerce_wp_checkbox( array( 'id' => '_myriad_link_enabled', 'label' => __('Enable link to Myriad product?', 'woothemes'), 'description' => __('Enable this option to enable linking this product to Myriad.', 'woothemes') ) ); 	
					  	  ?>
						  <div><strong>Subscription, Book Product, Circulation</strong></div>
						  <?php
						  ...
						  <div><strong>Book Product</strong></div>
						  <?php
      					  woocommerce_wp_select( array( 'id' => '_myriad_linked_bookrate', 'label' => __( 'Linked Book Product Rate', 'woocommerce' ), 'value' => get_post_meta(get_the_ID(), '_myriad_linked_bookrate')[0], 'options' => $pubController -> getBookRates($currentPublication,true), 'desc_tip' => true, 'description' => __( 'Myriad Book Product rate linked to this product', 'woocommerce' ) ) );
      					  ?>

      					  <div><strong>Circulation</strong></div>
      					  <?php
						  woocommerce_wp_select( array( 'id' => '_myriad_renewal_campaign', 'label' => __( 'Renewal Campaign', 'woocommerce' ), 'value' => get_post_meta(get_the_ID(), '_myriad_renewal_campaign')[0], 'options' => $pubController->getRenewalCampaigns(), 'desc_tip' => true, 'description' => __( 'Myriad Renewal Campaign  linked to this circulation', 'woocommerce' ) ) );
      					  ...
      					  <div><strong>Subscription, Book Product</strong></div>
      					  <?php
...
<?php 
   	...
   	public function myriad_process_product_meta( $post_id ) {
		...
		update_post_meta( $post_id, '_myriad_linked_bookrate', $_POST['_myriad_linked_bookrate']);
	     ...
	     // mail("h@hgs.so","Processing item",print_r($_product,true));exit();
			...
			// If this is a Myriad order then process it accordingly
	          if ( 'yes' == $_product->myriad_link_enabled) {
				...
				$myriadOrder->rate_id = (int)$_product->myriad_linked_rate > (int)$_product->myriad_linked_bookrate ? $_product->myriad_linked_rate : $_product->myriad_linked_bookrate;
		       ...
		       //  mail("simonpbriggs@gmail.com","Processing Myriad order",print_r($myriadOrder,true));
				$order_id = $orderController->processOrder($myriadOrder);

				if ($order_id)
				{
					 update_post_meta( $order->id, 'myriad-id', sanitize_text_field( $order_id ) );
				}
