...
class myriad_code_redemption_plugin{
	...
	public function __construct(){
		...
		add_shortcode("voucher_code_redemption_add_to_cart", array(&$this, "get_voucher_code_add_to_cart_form"));
	...
	public function get_voucher_code_redemption_form(){
		$error_part = "";
		if(count($_POST)){
			$error_part = $this->get_submit_action();
		}

		return $this->load_template_part('code_redemption') . $error_part;
	}

	public function get_voucher_code_add_to_cart_form(){
		if(isset($_GET['coupon_code']) && $_GET['coupon_code'] != "" && @$_GET['submit'] != "yes"){
			return "<a href=\""."http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]"."&submit=yes\">Add to cart</a>";
		}
		else if(isset($_GET['coupon_code']) && $_GET['coupon_code'] != "" && @$_GET['submit'] == "yes"){
			global $woocommerce;
			$voucher_code = $_GET['coupon_code'];

			$voucher = $this->wpdb->get_row(sprintf('SELECT * FROM '.$this->table_prefix.'myriad_voucher_code WHERE BINARY code="%s"', $voucher_code));

			if(!is_null($voucher) && (((int)$voucher->order_id == 0 && $voucher->number_of_uses == 0) || $voucher->number_of_uses > 0)){
	...
	private function get_submit_action(){
		...
		if(!is_null($voucher) && (((int)$voucher->order_id == 0 && $voucher->number_of_uses == 0) || $voucher->number_of_uses > 0)){
			...
			if($voucher_group->product_landing_page != ""){
				// print $voucher_group->product_landing_page . "?coupon_code=" . urlencode($voucher_code);exit();
				wp_redirect($voucher_group->product_landing_page . "?coupon_code=" . urlencode($voucher_code));
				return;
			}
			...
			return "<p>Voucher code was not found</p>";
		}
	}
	...
	public function register_voucher_code_settings(){
		 ...
		 */
			...
			if(isset($_POST) && count($_POST)){
				$this->wpdb->insert($this->table_prefix . 'myriad_voucher', array('group_id' => $_POST['group_name'], 'product_id' => $_POST['product_id'], 'coupon' => $_POST['discount'], 'payment_type_id' => $_POST['payment_type'], 'product_landing_page' => $_POST['group_landing']), array('%s', '%d', '%d', '%s', '%s'));
				wp_redirect('/wp-admin/options-general.php?page=myriad-voucher-code');
			}
		}
		/*
		 * Remote voucher code group
		 */
		else if(isset($_GET['state']) && $_GET['state'] == "remove"){
			$viewData['page_state'] = 'remove';

			if(isset($_GET['status']) && $_GET['status'] == "submit"){
				$this->wpdb->delete($this->table_prefix . 'myriad_voucher', array('id' => $_GET['voucher_id']), array('%d'));
				wp_redirect('/wp-admin/options-general.php?page=myriad-voucher-code');
			}
		}
		/*
				...
				switch($mode){
					case 'append':
						foreach($csv as $line){
							...
							if(isset($line[2])) $order_data['number_of_uses'] = (int)$line[2] > 0 ? (int)$line[2] : 1;
